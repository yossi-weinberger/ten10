...
| `recurring_total_count`  | `INTEGER`                           | Total number of installments for a recurring transaction       | Yes      | Only relevant if `is_recurring = true`, `NULL` otherwise. Supabase uses `recurring_total_count` (camelCase). |
...
- **TypeScript Type (`Transaction`):** The `Transaction` type in TypeScript (`src/types/transaction.ts`) now also uses `snake_case` for all its fields, matching the database schema. The `dataService.ts` layer ensures data is handled correctly with this consistent naming.
...
- **Load**: Triggered by authentication events (`AuthContext`) and data freshness checks. Fetches all relevant `transactions` from DB (SQLite via Tauri for Desktop, Supabase for Web using `dataService`) only if explicitly forced (e.g., on new login via `forceDbFetchOnLoad` flag) or if existing data in Zustand is stale (based on `lastDbFetchTimestamp`). Populates `transactions` array in Zustand store, replacing previous content if a fetch occurs.
