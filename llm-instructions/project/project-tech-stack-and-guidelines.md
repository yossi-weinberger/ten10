...
- **Toast Notifications**: **`sonner`** (via the wrapper `src/components/ui/sonner.tsx`) - The single toast library used across the app. Do not add `react-hot-toast` or the unused Radix `toast`/`toaster` primitives back in; `sonner` was consolidated as the only toast implementation.
  ...
  - Per-type colors (`success`/`error`/`warning`/`info`) are defined in `src/components/ui/sonner.tsx` using the app's own brand tokens (`--success`, `--destructive`, `--warning`, `--info` from `index.css`), not sonner's built-in `richColors` (which is intentionally **not** used — it ships its own generic palette unrelated to the brand). Follow the existing soft-tint convention (`bg-<token>/10` + `border-<token>/25`, e.g. as in `StatsCards.tsx`) when touching this styling, and keep icons as `lucide-react` components passed via the `icons` prop for visual consistency with the rest of the app.
