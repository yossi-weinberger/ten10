...
- **Toast Notifications**: **`sonner`** (via the wrapper `src/components/ui/sonner.tsx`) - The single toast library used across the app. Do not add `react-hot-toast` or the unused Radix `toast`/`toaster` primitives back in; `sonner` was consolidated as the only toast implementation.
  ...
  - The global `<Toaster>` lives in `App.tsx`. Its `theme` prop is wired to the app's own `useTheme` (`@/lib/theme`), **not** `next-themes` (not installed/used). Its `position` matches the **persistent desktop `Sidebar`'s side** (reading-start side: right in RTL, left in LTR — it's the first flex child and `dir="rtl"` mirrors flex visually), but its `offset` is computed in JS from the sidebar's *actual current width* — `App.tsx` already tracks `isSidebarExpanded` (collapsed `w-16` vs hover-expanded `w-44`) plus a `useMediaQuery("(min-width: 768px)")` check (the sidebar is `hidden` below `md`, replaced by the mobile `Sheet`) — so the toast is shifted just past whichever width applies at that moment and never sits on top of the sidebar in either state. Don't rely on sonner's own internal mobile detection for this (its `mobileOffset` prop switches based on its own CSS media query, which may not be loaded/may not match Tailwind's `md`, and can't react to `isSidebarExpanded` anyway). This positioning also naturally keeps the toast clear of `ContactFAB`, which sits on the opposite side.
  - Per-type colors (`success`/`error`/`warning`/`info`) are defined in `src/components/ui/sonner.tsx` using the app's own brand tokens (`--success`, `--destructive`, `--warning`, `--info` from `index.css`), not sonner's built-in `richColors` (which is intentionally **not** used — it ships its own generic palette unrelated to the brand). These use `color-mix(in srgb, hsl(var(--token)) X%, hsl(var(--background)))` rather than a plain `/10` opacity utility — a toast floats over arbitrary content, so a translucent background lets whatever's behind it bleed through; `color-mix()` produces a fully opaque solid color instead, while still adapting automatically to light/dark theme. Keep the `!` (important) modifier on these — they target the same properties (`background-color`/`border-color`) as the base `toast` classNames with equal Tailwind specificity, so without `!` the winner between the two is undefined. Keep icons as `lucide-react` components passed via the `icons` prop for visual consistency with the rest of the app.
