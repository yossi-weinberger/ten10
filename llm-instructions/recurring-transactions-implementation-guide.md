...
- **עדכון תצוגת טבלת הטרנזקציות:**
  ...
  - **הפתרון:** יש לעדכן את פקודת ה-Rust האחראית על שליפת התנועות (`get_filtered_transactions_handler`) כך שתבצע `LEFT JOIN` עם טבלת `recurring_transactions`. לאחר מכן, יש לעדכן את קומפוננטות ה-UI ב-Frontend כדי לצרוך ולהציג את המידע הנוסף.
