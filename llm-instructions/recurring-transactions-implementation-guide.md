...
כתוצאה מהשינויים במבנה הנתונים, יש לבצע התאמות קריטיות בתצוגת הנתונים ובפונקציונליות הייצוא בגרסת הדסקטופ.

- **עדכון תצוגת טבלת הטרנזקציות:**

  - **הבעיה:** הטבלה הראשית אינה מציגה מידע מפורט על הוראות קבע (כמו ה-Badge, סטטוס, והתקדמות "X מתוך Y"), כפי שקיים בגרסת הווב.
  - **הפתרון (מעודכן):**
    - **צד-שרת (Rust):** ✅ **בוצע.** פקודת `get_filtered_transactions_handler` עודכנה (כחלק מתיקון המסננים, ראה סעיף 4.6) וכעת היא מבצעת `LEFT JOIN` עם טבלת `recurring_transactions` ומספקת את כל המידע הדרוש.
    - **צד-לקוח (Frontend):** ⏳ **לביצוע.** יש לעדכן את קומפוננטת `TransactionRow.tsx` כדי שתצרוך את המידע הנוסף ותציג אותו ויזואלית (Badge, Tooltip וכו') בגרסת הדסקטופ.

- **בדיקה ותיקון של פונקציונליות הייצוא:**
  - **הבעיה:** סביר להניח שפונקציות הייצוא ל-Excel, PDF ו-CSV נשברו, כיוון שהן מסתמכות על מבנה הנתונים הישן.
  - **הפתרון:** יש לעדכן את הלוגיקה של כל אחד ממנגנוני הייצוא כדי שתתמודד עם מבנה הנתונים החדש שמתקבל מה-`JOIN`.
...
כדי לאפשר בדיקה יעילה של המנגנון, נדרש להוסיף כלי דיבוג שיאפשרו להריץ את התהליכים באופן יזום.
