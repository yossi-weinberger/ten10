...
- **פתרון:** יש להריץ את הפקודה `execute_due_recurring_transactions_handler` בעת עליית האפליקציה. רצוי להריץ אותה ב-thread נפרד ב-Rust כדי לא להקפיא את ה-UI. במקביל, ניתן להציג חיווי בממשק המשתמש (למשל, "מעדכן הוראות קבע...") כל עוד התהליך רץ ברקע.
