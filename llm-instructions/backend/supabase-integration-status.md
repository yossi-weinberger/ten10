...
- **Branches and CI/CD:** Staging: apply migrations manually (MCP or Dashboard). Production: migrations via `deploy-supabase-migrations.yml` on **PR open** (and push to main); functions via `deploy-supabase-functions.yml` on push to `main`. See `supabase/BRANCHES_AND_CI_CD_MAP.md`.
