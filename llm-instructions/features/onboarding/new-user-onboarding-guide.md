...
First-run steps: `home-intro` → `tithe-balance` → `opening-balance` → `card-quick-add` → `date-range` → `continue-to-form` → `transaction-form` → `transaction-basics` → `transaction-flags` → `recurring-toggle` → `live-balance` → `transaction-import`. Date range comes after the cards. Import is last on the form so one transaction is learned first. Opening the starting-balance modal destroys the Driver overlay and resumes at `card-quick-add`. Chart and Contact are mentioned only in the success copy.

Import help tour (manual, `/transactions-table/import`): `import-intro` → `import-steps` → `import-template` → `import-upload` → `import-mapping` → `import-review` → `import-approve`. Reaching `import-upload` moves the wizard to the upload screen. Later screens stay `skipMissingElement: false` so Driver does not treat them as the end of the tour before they mount. Next on upload confirms the selected sheet, or loads the TEN10 sample template, and immediately moves the highlight to mapping so Driver is not left on an unmounted target. Next on mapping builds the preview the same way and waits for the review screen. There is no Analytics Driver tour — the Home checklist only links to `/analytics`.

Page help: a `CircleHelp` button next to the title on Home, Add Transaction, and Import starts only that page's tour. The Home help tour keeps `continue-to-form` and then starts the form help tour. Hidden while any tour or Welcome is open. Settings restart opens Welcome first, then the full first-run if the user starts it.
