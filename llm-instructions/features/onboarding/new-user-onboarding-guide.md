...
Import help tour (manual, `/transactions-table/import`): `import-intro` → `import-steps` → `import-template` → `import-upload` → `import-mapping` → `import-review` → `import-approve`. Reaching `import-upload` moves the wizard to the upload screen. Next on upload confirms the selected sheet, or loads the TEN10 sample template, and immediately moves the highlight to mapping so Driver is not left on an unmounted target. Next on mapping builds the preview the same way and waits for the review screen. There is no Analytics Driver tour — the Home checklist only links to `/analytics`.

Page help: a `CircleHelp` button next to the title on Home, Add Transaction, and Import starts only that page's tour. The Home help tour keeps `continue-to-form` and then starts the form help tour. Hidden while any tour or Welcome is open. Settings restart still replays the full first-run.
