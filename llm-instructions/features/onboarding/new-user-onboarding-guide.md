...
Import help tour (manual, `/transactions-table/import`): `import-intro` → `import-steps` → `import-checklist` → `import-template` → `import-upload` → `import-mapping` → `import-review` → `import-approve`. Reaching `import-upload` moves the wizard to the upload screen. Next on upload confirms the selected sheet, or loads the TEN10 sample template if there is no file, then the tour follows mapping / review on the real screens. There is no Analytics Driver tour — the Home checklist only links to `/analytics`.
