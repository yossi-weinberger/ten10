...
- **ממשק משתמש:** דרך `TransactionForm.tsx` (ב-`src/components/forms/TransactionForm.tsx`).
  ...
  - **לוגיקה (Frontend):**
    ...
    - **עריכה:** שינוי `day_of_month` מעדכן `next_due_date` ל-`rescheduleBillingDayInMonth(next_due_date, day_of_month)` — אותו חודש, יום חדש (גם כשמזיזים אחורה). ה-cron ימשיך catch-up מהמועד המעודכן.
...
- **תזמון:** Cron Job יומי (בחצות UTC) דרך Supabase, קורא ל-Edge Function `process-recurring-transactions`. כתובת ה-URL של ה-Function נשמרת ב-Supabase Vault (סוד `functions_base_url`) כדי לתמוך גם בבראנצ'ים; ראה `supabase/MIGRATION_VAULT_SETUP.md`.
  - **לוגיקה (Edge Function - TypeScript):**
    ...
    - **לכל תנועה בלולאה:**
      1. **בדיקת פרטי המרה שמורים:** אם `original_amount` ו-`original_currency` אינם null:
         - **שער ידני (`rate_source === "manual"`):** תמיד משתמש בשער שנקבע בעת ההגדרה.
         - **שער אוטומטי (`rate_source === "auto"`):** מנסה למשוך שער עדכני מה-API (עם cache לכל ריצה למניעת קריאות כפולות). אם הצליח — משתמש בשער החדש. אם נכשל (כל הספקים נכשלו) — נסוג לשער השמור מעת ההגדרה.
      2. **Fallback להמרה דינמית:** אם אין פרטי המרה שמורים ומטבע ההוראה שונה מהמטבע הראשי, קורא ל-API חיצוני לקבלת שער עדכני (עם cache).
...
- **Allowed Web/Desktop edit fields:** the same jsonb patch as transactions — `payment_method`, `category`, `description`, `recipient`, `is_chomesh`. Key presence writes the field; untouched fields are omitted. Bulk `status` editing is deferred until occurrence creation and recurring-state advancement are atomic together, avoiding a race with recurring execution; both backends reject a `status` key.
- **Blocked selections:** Rows with `status === "completed"` block every bulk-edit field. Category requires one income/expense family. Recipient requires every row to be `donation` or `non_tithe_donation`. Chomesh requires every row to share the same allowed type.
...
מסמך זה מעודכן נכון לאוגוסט 2026; עודכן לאחרונה עם תיעוד bulk edit v2 (jsonb patch, שדות עצמאיים כולל description/recipient/chomesh, ללא עריכת `status`).
