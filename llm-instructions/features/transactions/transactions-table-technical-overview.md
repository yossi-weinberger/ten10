...
- **UI:** `BulkActionsToolbar.tsx` (toolbar when `selectedCount > 0`), `BulkEditDialog.tsx` (form shell), `BulkEditFields.tsx` (independent fields). Fields start **untouched**; submit sends one JSON patch of touched fields only. No Undo after bulk delete/update. No PostHog / `trackProductEvent` calls in bulk UI flows.
- **Adaptive selected-row actions:** one selected row keeps the toolbar visible but routes Edit/Delete to the existing singular handlers (including opening-balance and completed-recurring safeguards), while two or more selected rows use the atomic bulk handlers; cancelling a singular or bulk modal preserves the selection, and a successful save clears it.
...
- **Allowed bulk edit fields (both tables):** `payment_method` (50), `category` (50), `description` (100), `recipient` (50), `is_chomesh` (boolean). Key presence in `TransactionBulkPatch` means write; JSON `null` clears a text field; missing keys stay unchanged. `assertBulkPatch` / `buildBulkPatch` live in `src/lib/tableTransactions/bulkActions.ts`. Category requires a **homogeneous category family** (`getBulkCategoryFamily`: income = `income`/`exempt-income`; expense = `expense`/`recognized-expense`). Recipient requires every row to be `donation` or `non_tithe_donation` (`getBulkRecipientFamily`). Chomesh requires every row to share the **same exact type** in `{income, donation, recognized-expense}` (`getBulkChomeshType`). Plain `expense` cannot receive bulk chomesh because balances ignore `is_chomesh` unless the type is `recognized-expense`. **Blockers:** any transaction `type === "initial_balance"` or any recurring `status === "completed"` blocks **all** bulk edit fields (bulk delete is still allowed, with the existing transaction warnings). Mixed category families hide category only. Recurring `status` cannot be patched.
- **Allowed bulk edit fields (recurring):** same field set and rules as transactions. Recurring `status` bulk editing is deferred until recurring occurrence creation and state advancement are atomic together; both backends reject a `status` key.
