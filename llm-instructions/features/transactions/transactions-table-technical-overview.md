...
- **Store behaviour:** bulk delete/update actions set `bulkLoading`, call the service once, then await one refetch before resolving. A failed refetch sets both the table error and `bulkError`, rejects the action, and leaves the edit modal open. On success, `TransactionsTableDisplay` / `RecurringTransactionsTableDisplay` close `BulkEditDialog`, then schedule `clearSelection` and the success toast in `requestAnimationFrame`.
...
- **Main Actions:**
  ...
  - `deleteTransactionsBulk(ids, platform)` / `updateTransactionsBulk(ids, change, platform)`:
    - Atomic bulk delete/update via data-layer RPC/Tauri handlers. **Not optimistic** — both actions refetch from the store exactly once before resolving. Guards against concurrent bulk actions via `bulkLoading`.
