...
- **בדיקה ותיקון של פונקציונליות הייצוא:**
  ...
  - **המשך (דסקטופ):** שמירת קבצים מהטבלה (CSV/Excel/PDF) נעשית דרך `src/lib/utils/save-export-file.ts` — דיאלוג שמירה native (`dialog.save`) כמו בייצוא PDF של האנליטיקה; ביטול הדיאלוג מסומן ב-`EXPORT_DESKTOP_SAVE_CANCELLED` בלי toast הצלחה.
