...
/** Adds geresh or gershayim, so 1 is א׳ and 15 is ט״ו. */
export function formatHebrewNumeral(value: number): string {
  const numeral = toHebrewNumeral(value);
  if (numeral.length <= 1) {
    return `${numeral}׳`;
  }
...
export function formatHebrewYear(year: number): string {
  return formatHebrewNumeral(year % 1000);
}
