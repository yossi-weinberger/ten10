...
export async function restoreDesktopSettings(): Promise<RestoredDesktopSettings> {
  ...
  try {
    ...
    if (isValidCurrency(storedCurrency)) {
      logger.log(`DesktopSettingsService: Restored default_currency from SQLite: ${storedCurrency}`);
      ...
      if (isValidCurrency(inferred)) {
        logger.log(`DesktopSettingsService: Inferred default_currency from transactions: ${inferred}`);
        store.updateSettings({ defaultCurrency: inferred });
        await invoke("set_default_currency", { currency: inferred });
      } else if (isValidCurrency(settings.defaultCurrency) && settings.defaultCurrency !== "ILS") {
        await invoke("set_default_currency", { currency: settings.defaultCurrency });
      }
    }
    ...
    if (isValidLanguage(storedLang)) {
      logger.log(`DesktopSettingsService: Restored language from SQLite: ${storedLang}`);
      store.updateSettings({ language: storedLang });
    } else if (isValidLanguage(settings.language) && settings.language !== "he") {
      await invoke("set_app_setting", { key: "language", value: settings.language });
    }

    // --- Theme ---
    const storedTheme = await invoke<string | null>("get_app_setting", { key: "theme" });
    if (isValidTheme(storedTheme)) {
      logger.log(`DesktopSettingsService: Restored theme from SQLite: ${storedTheme}`);
      store.updateSettings({ theme: storedTheme });
      result.theme = storedTheme;
    } else if (isValidTheme(settings.theme) && settings.theme !== "system") {
      await invoke("set_app_setting", { key: "theme", value: settings.theme });
    }
    ...
    if (storedTimeout !== null && storedTimeout !== undefined) {
      ...
      if (!isNaN(num) && num >= 0) {
        logger.log(`DesktopSettingsService: Restored autoLockTimeoutMinutes from SQLite: ${num}`);
        store.updateSettings({ autoLockTimeoutMinutes: num });
      }
    } else if (settings.autoLockTimeoutMinutes !== undefined && settings.autoLockTimeoutMinutes !== 10) {
