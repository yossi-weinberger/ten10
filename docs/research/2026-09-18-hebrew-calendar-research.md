...
**תאריך:** 18.9.2026 (ז' בתשרי תשפ"ז) · **גרסת אפליקציה:** 0.7.5 · **סטטוס ביצוע (23.9.2026):** P1–P4, 0a–0b ושלבים 1–3 הושלמו ואומתו ב-[Draft PR #424](https://github.com/yossi-weinberger/ten10/pull/424): CI ירוק בשני אזורי זמן, testing מבודד, Preview מחובר ישירות ל-testing, Temporal parity עבר ב-Vite וב-Deno, ו-471 בדיקות ירוקות.
...
| **2. רמה A — תצוגה — הושלם** | `formatDisplayDate` ו-`useDisplayDate` חוברו לטבלאות, הוראות קבע, dashboard/analytics, heatmap, import, changelog והלכה. תאריך משני מוצג כשורה/טקסט משני בלבד. CSV/Excel שומרים Gregorian ומוסיפים Hebrew לפי ההגדרה; PDF מציג Gregorian+Hebrew בתא בטוח. grouping, filters ו-RPCs נשארו Gregorian | 28 קבצים, 454 tests; browser smoke הציג `13 באלול 5786 · 26/08/2026` מול testing בלבד | הושלם | 1 |
| **3. גריד עברי — הושלם** | `react-day-picker` משתמש ב-Hebrew DateLib אמיתי: 29/30 ימים, 12/13 חודשים, אדר א'/ב', ניווט/dropdowns, RTL ונגישות. בחירה מחזירה Gregorian `Date`; קלט ידני נשאר `DD/MM/YYYY` עם הסבר מפורש. Browser smoke: תשרי 5787 הוצג עם 30 ימים; בחירת א' תשרי מילאה `12/09/2026` | DateLib + 17 focused tests + jsdom/RTL | הושלם | 1 |
