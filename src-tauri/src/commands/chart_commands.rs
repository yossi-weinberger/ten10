...
pub fn get_desktop_monthly_financial_summary(
    ...
    let end_date_actual = NaiveDate::parse_from_str(&end_date_str, "%Y-%m-%d")
        .map_err(|e| {
            eprintln!("[Rust Chart] Error parsing end_date_str '{}': {}", end_date_str, e);
    ...
    let start_date_actual = end_month_start_date.checked_sub_months(Months::new((num_months -1) as u32)).unwrap();
    
    println!("[Rust Chart] Calculated start_date_actual (first day of period): {}", start_date_actual);
    println!("[Rust Chart] Calculated end_month_start_date (first day of last month in period): {}", end_month_start_date);
    ...
    while current_month_iter <= end_month_start_date {
        ...
        let next_month_start = current_month_iter.checked_add_months(Months::new(1)).unwrap();
        let month_end_str = next_month_start.checked_sub_days(chrono::Days::new(1)).unwrap().format("%Y-%m-%d").to_string();
        ...
        let mut rows = stmt.query(params![month_start_str, month_end_str]).map_err(|e| {
            eprintln!("[Rust Chart] Error executing SQL query for month {}: {}", month_label_str, e);
            e.to_string()
        })?;

        if let Some(row) = rows.next().map_err(|e| {
            eprintln!("[Rust Chart] Error fetching row for month {}: {}", month_label_str, e);
            e.to_string()
        })? {
            ...
            eprintln!("[Rust Chart] No row returned from query for month {}, adding zeroed entry.", month_label_str);
             results.push(DesktopMonthlyDataPoint {
                month_label: month_label_str.clone(),
                income: 0.0,
                donations: 0.0,
    ...
    println!("[Rust Chart] Final results ({} items): {:?}", results.len(), results);
    Ok(results)
} 